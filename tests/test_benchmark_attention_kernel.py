...
def manifest(include_packed_dequant_flash):
    ...
    q = torch.empty(1, 2, 4)
    k_reference = torch.empty(1, 8, 1, 4)
    k_int8 = torch.empty(1, 8, 1, 4, dtype=torch.int8)
    scale = torch.empty(1, 8, 1, dtype=torch.float16)
    block_tables = torch.zeros(1, 1, dtype=torch.int32)
    context_lens = torch.full((1,), 8, dtype=torch.int32)
